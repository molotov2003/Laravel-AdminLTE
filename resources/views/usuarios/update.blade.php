...
@section('content')
  ...
  <form action="{{route('dashboard.editarUser', $usuario)}}" method="POST"  class="formularioCrear"  id="formularioCrear">
    ...
    <div class="mb-3">
      ...
      <input type="text" name="name" class="form-control" id="exampleInputEmail1" value="{{$usuario->name}}" aria-describedby="emailHelp">   
    </div>
    @error('name')
    <span>{{$message}}</span> 
@enderror
    <div class="mb-3">
      ...
      <input type="text" name="email" class="form-control" value="{{$usuario->email}}" id="exampleInputPassword1">
    </div>
    <div class="mb-3">
